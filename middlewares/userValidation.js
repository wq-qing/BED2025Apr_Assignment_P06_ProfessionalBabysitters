...
const registerUserSchema = Joi.object({
  Id: Joi.string().trim().min(1).max(10).required().messages({
  ...
  Role: Joi.string().valid("Doctor", "Elderly").max(20).required().messages({
    "string.base": "Role must be a string",
    "any.only": "Role must be one of: Doctor, Elderly",
